...
def l2_regularization(weights: NDArray, lambd: float) -> NDArray:
    return lambd * np.power(weights, 2).sum()

def total_criterion(input: NDArray, target: NDArray, weights: NDArray, lambd: float) -> NDArray:
    return sample_criterion(input, target) + l2_regularization(weights, lambd)
